...
// Import API utilities for better error handling
import { HDI_API, ElevenLabs } from './api-utils';
...
const HoustonVoiceAI = () => {
  ...
  useEffect(() => {
    HDI_API.testConnection().then(connected => {
  ...
  const speakWithElevenLabs = async (text) => {
    try {
      ...
      const audioBlob = await ElevenLabs.textToSpeech(text, {
  ...
  const processVoiceQuery = async (query) => {
    ...
    try {
      ...
      if (isPropertyQuery(query)) {
        const propertyData = await HDI_API.voiceSearchProperty(query);
        if (propertyData && propertyData.length > 0) {
          const formattedData = await HDI_API.getVoiceFormattedProperty(propertyData[0].id);
          response = formattedData.voice_summary || formattedData.summary || 'Property found but no voice summary available.';
        } else {
          const aiResponse = await HDI_API.askAIWithContext(query, conversationHistory);
          response = aiResponse.response || aiResponse.answer || 'No response received.';
        }
      } else {
